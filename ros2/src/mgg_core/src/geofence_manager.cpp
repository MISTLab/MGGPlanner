...
std::string gfmt(const char* f, Args... args) {
  char buf[256];
  std::snprintf(buf, sizeof(buf), f, args...);
  return std::string(buf);
}
std::string gfmt(const char* f) { return std::string(f); }
}  // namespace
...
void GeofenceManager::addGeofenceArea(Polygon2d& poly, bool merge) {
  ...
  logWarn(gfmt("Number of geofence areas: %d", (int)geofence_list_.size()));
}
      ...
      for (int j = i + 1; j < geofence_list_.size(); ++j) {
        if ((geofence_list_[i].polygon.doIntersect(
          ...
          if (union_poly.size() == 1) {
            ...
            logWarn(gfmt("Polygon with inner ring or they don't intersect."));
          }
        }
      }
      if (redo) break;
