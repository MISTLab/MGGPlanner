...
void Graph::clear() {
  // The ROS 1 graph.h declared clear() but no translation unit ever defined
  // it. That linked only because nothing called it: GraphManager::reset()
  // discards the whole Graph and constructs a new one instead. Anything that
  // did call it would have failed at link time, so define it here rather than
  // carry the trap across.
  graph_.clear();
