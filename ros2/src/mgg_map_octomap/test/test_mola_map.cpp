...
std::vector<Voxel> roadVoxels() {
  std::vector<Voxel> road;
  for (std::int64_t x = -10; x <= 10; ++x)
    for (std::int64_t y = -4; y <= 4; ++y) road.push_back({x, y, 0});
  return road;
}

mgg::RasterParams rasterParams() {
  mgg::RasterParams params;
  params.cell_size_m = 0.5;
  params.body_radius_m = 0.2;
  params.max_step_height_m = 0.15;
  params.body_height_m = 0.6;
  return params;
}

TEST(MolaMap, TraversabilityRasterIsCachedPerSnapshotAndRebuiltForASuccessor) {
  Publication publication;
  MolaMap provider(config(publication));
  EXPECT_EQ(provider.traversability(rasterParams()), nullptr);
  const auto first = publication.publish(0, roadVoxels(), freeBlock());
  provider.requestSnapshot(first);
  ASSERT_TRUE(waitFor([&]() { return provider.getStatus(); }))
      << provider.lastError();
  const auto loaded_generation = provider.activeGeneration();

  const auto raster = provider.traversability(rasterParams());
  ASSERT_NE(raster, nullptr);
  EXPECT_TRUE(raster->consistent());
  // Same snapshot, same parameters: the same raster object.
  EXPECT_EQ(provider.traversability(rasterParams()).get(), raster.get());
  // The road: free at the surface height, (0 + 0.5) * 0.2 m.
  EXPECT_EQ(raster->stateAtXY(Eigen::Vector2d(0.25, 0.25)),
            mgg::RasterCellState::kFree);
  EXPECT_NEAR(raster->groundAtXY(Eigen::Vector2d(0.25, 0.25)), 0.1, 1e-9);
  EXPECT_EQ(raster->stateAtXY(Eigen::Vector2d(1.1, 0.1)),
            mgg::RasterCellState::kFree);
  EXPECT_EQ(raster->stateAtXY(Eigen::Vector2d(5.0, 5.0)),
            mgg::RasterCellState::kUnknown);

  // Other parameters rebuild; the snapshot caches one raster at a time.
  mgg::RasterParams coarse_params = rasterParams();
  coarse_params.cell_size_m = 1.0;
  const auto coarse = provider.traversability(coarse_params);
  ASSERT_NE(coarse, nullptr);
  EXPECT_NE(coarse.get(), raster.get());
  EXPECT_NEAR(coarse->cell_size, 1.0, 1e-12);
  EXPECT_EQ(provider.traversability(coarse_params).get(), coarse.get());
  const auto again = provider.traversability(rasterParams());
  ASSERT_NE(again, nullptr);
  EXPECT_NE(again.get(), raster.get());
  EXPECT_EQ(again->width, raster->width);

  // A successor snapshot (a wall appeared) builds its own raster; the earlier
  // raster is immutable and still describes the earlier geometry.
  auto with_wall = roadVoxels();
  with_wall.push_back({5, 0, 1});
  with_wall.push_back({5, 0, 2});
  const auto successor = publication.publish(1, with_wall, freeBlock());
  provider.requestSnapshot(successor);
  ASSERT_TRUE(waitFor([&]() {
    return provider.activeGeneration() != loaded_generation;
  })) << provider.lastError();
  const auto rebuilt = provider.traversability(rasterParams());
  ASSERT_NE(rebuilt, nullptr);
  EXPECT_NE(rebuilt.get(), again.get());
  EXPECT_EQ(rebuilt->stateAtXY(Eigen::Vector2d(1.1, 0.1)),
            mgg::RasterCellState::kObstacle);
  EXPECT_EQ(again->stateAtXY(Eigen::Vector2d(1.1, 0.1)),
            mgg::RasterCellState::kFree);
  EXPECT_EQ(provider.traversability(rasterParams()).get(), rebuilt.get());
  {
    // Within a read lease every call sees the admitted snapshot's raster.
    auto lease = provider.acquireReadLease();
    EXPECT_EQ(provider.traversability(rasterParams()).get(), rebuilt.get());
  }
  // Retracting the snapshot removes the raster with it.
  provider.resetMap();
  EXPECT_EQ(provider.traversability(rasterParams()), nullptr);
}

TEST(MolaMap, TraversabilityRasterIsInTheNavigationFrameAndDiscsAreExposed) {
  Publication publication;
  Eigen::Isometry3d component_from_navigation = Eigen::Isometry3d::Identity();
  component_from_navigation.linear() =
      Eigen::AngleAxisd(M_PI / 2.0, Eigen::Vector3d::UnitZ()).toRotationMatrix();
  ...
  with_wall.push_back({5, 0, 1});
  with_wall.push_back({5, 0, 2});
  const auto request = publication.publish(0, with_wall, freeBlock(), true,
                                           component_from_navigation);
  MolaMap provider(config(publication));
  provider.requestSnapshot(request);
  ASSERT_TRUE(waitFor([&]() { return provider.getStatus(); }))
  ...
  const Eigen::Isometry3d navigation_from_component =
      component_from_navigation.inverse();
  // A road surface sample in the component frame lands in the navigation
  // frame at the transformed XY with the transformed height.
  const Eigen::Vector3d road =
      navigation_from_component * Eigen::Vector3d(0.1, 0.1, 0.1);
  EXPECT_EQ(raster->stateAtXY(road.head<2>()), mgg::RasterCellState::kFree);
  EXPECT_NEAR(raster->groundAtXY(road.head<2>()), road.z(), 1e-9);
  const Eigen::Vector3d wall =
      navigation_from_component * Eigen::Vector3d(1.1, 0.1, 0.1);
  EXPECT_EQ(raster->stateAtXY(wall.head<2>()), mgg::RasterCellState::kObstacle);
  // The component-frame location itself is not a road cell of this raster.
  EXPECT_NE(raster->stateAtXY(Eigen::Vector2d(1.1, 0.1)),
            mgg::RasterCellState::kObstacle);

  // Transient discs are reported in the navigation frame until they expire.
  EXPECT_TRUE(provider.transientDiscs().centres.empty());
  provider.setTransientDiscs({Eigen::Vector2d(0.3, 0.35)}, 0.25, 60.0);
  const auto discs = provider.transientDiscs();
  ASSERT_EQ(discs.centres.size(), 1u);
  EXPECT_NEAR(discs.centres.front().x(), 0.3, 1e-12);
  EXPECT_NEAR(discs.centres.front().y(), 0.35, 1e-12);
  EXPECT_NEAR(discs.radius_m, 0.25, 1e-12);
  provider.setTransientDiscs({Eigen::Vector2d(0.3, 0.35)}, 0.25, 0.05);
  std::this_thread::sleep_for(std::chrono::milliseconds(120));
  EXPECT_TRUE(provider.transientDiscs().centres.empty());
  provider.setTransientDiscs({}, 0.25, 60.0);
